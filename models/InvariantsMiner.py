...
class InvariantsMiner(object):
    ...
    def __init__(self, percentage=0.98, epsilon=0.5, longest_invarant=None, scale_list=[1,2,3,4,5]):
    ...
    def anomalies_invariants(self, X, ids):
        """ Root cause analysis with mined invariants
        ...
        """
        # IM : Invariant Mining
        anomalies_IM = pd.Series(np.zeros(X.shape[0])).replace(0, '')
        thresholdIMcols = 1e-6 / X.shape[1] # 3, 1e-6 / X.shape[1]
        for cols, theta in self.invariants_dict.items():
            # Calculate the values of invariants
            IMcols = np.fabs(np.dot(X[:, cols], np.array(theta)))
            # Save the non-zero values of the abnormal invariants
            IMcolssum = pd.Series(IMcols)
            IMcolssum[IMcolssum.values<=thresholdIMcols] = 0
            IMcolssum = IMcolssum.replace(0, '').astype(str)
            # IMcolssum = pd.Series(IMcols).replace(0, '').astype(str)
            # Infer whether each invariant has an exception
            IMcols = (IMcols > thresholdIMcols).astype(int)
            cols2ids = (ids[cols[0]], ids[cols[1]])
            # Save the abnormal invariants and the corresponding values
            # anomalies_IM += pd.Series(IMcols).replace([0, 1], ['', ',{0}:{1}='.format(cols, theta)]) + IMcolssum
            # anomalies_IM += pd.Series(IMcols).replace([0, 1], ['', ',{0}'.format(cols)])
            anomalies_IM += pd.Series(IMcols).replace([0, 1], ['', ',{0}:{1}='.format(cols2ids, theta)]) + IMcolssum
            # anomalies_IM += pd.Series(IMcols).replace([0, 1], ['', ',{0}'.format(cols2ids)])
        return anomalies_IM

    def filterby2invariants(self, data_df, numinvars):
        """
    ...
    def _estimate_invarant_space(self, X):
        ...
        for i in range(num_events - 1, -1, -1):
            ...
            if zero_count / float(num_instances) < self.percentage:
                break
            r += 1
        print('Invariant space dimension : {} (without p=3)'.format(r))
    ...
    def _invariants_search(self, X, r):
        ...
        while len(item_list) != 0:
            ...
            if FLAG_break_loop:
                break
            length += 1
        # Page 9 in Ref. [ATC'10] Mining Invariants from Console Logs for System Problem Detection,
        # by Jian-Guang Lou, Qiang Fu, Shengqi Yang, Ye Xu, Jiang Li. [Microsoft]
        # Eliminate invariants whose absolute value of the coefficient is greater than p=3
        p = 3
        invariants_dict = {k:v for k,v in invariants_dict.items() if max(np.fabs(v))<=p}
        print('Invariant space dimension : {} (p=3)'.format(len(invariants_dict)))
        print('Mined {} invariants: {}\n'.format(len(invariants_dict), invariants_dict))
        print('* The meaning of (i, j): [ci, cj] is as follows:')
        print('* (i-th template, j-th template): [coefficient of the i-th template, coefficient of the i-th template],')
        print('* -p <= ci,cj <= p, where p is a positive integer.')
        print('* Refer to p=3 in Page 9 of Ref. Mining Invariants from Console Logs for System Problem Detection,')
        print('* by Jian-Guang Lou, Qiang Fu, Shengqi Yang, Ye Xu, Jiang Li. ATC 2010, [Microsoft].\n')
